...
print(SCAN_NAME, args.only_one_slice)
...
images = sorted(glob.glob(os.path.join(data_folder, "*_ct.nii.gz")))[:15] #OMM
labels = sorted(glob.glob(os.path.join(data_folder, "*_seg.nii.gz")))[:15] #OMM
# =====
keys = ("image", "label")
...
for idx_mini_batch,mini_batch in enumerate(train_loader):
    ...
    # HYPERPARAMETER
    aaa=0
    for BACKGROUND_INTENSITY in [0.01, .11, 1]:
        for STEP_SIZE in [0.03, 0.1, .3, 1, 5]:
            for SCALE_MASK in [0.03, 0.1, .3, 1, 5]:
                for SEED_VALUE in [.05, .11, .19, 1]:
                    for CH0_1 in [0, 1]:
                        for CH1_16 in [1, 16]:
                            for ALIVE_THRESH in [.03, .1, .5]:
                                aaa+=1
                                if aaa <= 21:continue
                                print(f'aaa={aaa}')
                                ...
                                path_parent = '/content/drive/My Drive/Datasets/covid19/COVID-19-20_augs_cea/CeA_0x'
                                path_save_synthesis_parent = f'{path_parent}_grow={GROW_ON_K_ITER}_bg={BACKGROUND_INTENSITY:.02f}_step={STEP_SIZE}_scale={SCALE_MASK}_seed={SEED_VALUE}_ch0_1={CH0_1}_ch1_16={CH1_16}_ali_thr={ALIVE_THRESH}/'
                                ...
                                for idx_lesion, (target, coord, mask, this_seed) in enumerate(zip(targets, coords, masks, seeds)):
                                    ...
                                    print(f'==== LESION {idx_mini_batch}/{len(train_ds)} CLUSTER {idx_lesion}/{len(coords)}. {name_prefix}')
                                    # prepare seed
                                    seed, seed_tensor, seed_pool = prepare_seed(target, this_seed, device, num_channels = num_channels, pool_size = 1024)

                                    # initialize model
                                    model = CeA_0x(device = device, grow_on_k_iter=GROW_ON_K_ITER, background_intensity=BACKGROUND_INTENSITY, 
                                    step_size=STEP_SIZE, scale_mask=SCALE_MASK, ch0_1=CH0_1, ch1_16=CH1_16, alive_thresh=ALIVE_THRESH)
